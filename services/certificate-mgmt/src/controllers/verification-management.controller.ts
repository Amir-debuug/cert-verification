...
import {parseSignature} from '../services/parser.utils';
 ...
 * by Verification Management.
 *
 * Resources in this group are related to signing and verification.
 */
@api({
  components: {
    requestBodies: {
...
export class VerificationManagementController {
  constructor(
    @inject(SessionServiceBindings.SESSION_SERVICE)
    public sessionService: SessionService,
    @inject(VerificationServiceBindings.VERIFICATION_SERVICE)
  ...
  /**
   * Provides verification capability of scanned certificate.
   *
   * @param xTracifierClientSignature Device signature
   * @param _requestBody Verify a scanned certificate
   * @returns Certificate verification status
   */
  @operation('post', '/verification-mgmt/verify/certificate', {
  ...
  async verifyCertificate(
    ...
    xTracifierClientSignature: string,
    @requestBody({
      description: 'Verify a scanned certificate',
      content: {
        'application/json': {
    ...
    // parse signature
    const signature: Signature = parseSignature(xTracifierClientSignature);

    // verify document
    const result = await this.verificationService.verifyDocument(_requestBody);

    // save scanned certificate when valid
    if (result.isValid) {
      await this.dataManagementService.newScannedCert(
        currentUser,
        signature,
      );
    }
